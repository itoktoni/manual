<x-layout>
    <x-card :model="$model">
        <x-form :model="$model">
            ...
            <x-input  name="tanggal" type="date" :col="3" value="{{ $model->retur_tanggal ?? date('Y-m-d') }}" label="Tanggal" />
            ...
            <div class="col-12">
                ...
                <table class="table">
                    ...
                    <tbody id="jenis-tbody">
                        ...
                        @php
                        $single = $transaksi ? $transaksi->where('retur_id_jenis', $key)->first() : null;
                        @endphp
                        <tr>
                            <input type="hidden" name="qty[{{ $key }}][retur_id]" value="{{ $single->retur_id ?? null }}" />
                            <td class="text-center">{{ $loop->iteration }}</td>
                            <td>{{ $value }}</td>
                            <td data-label="Kotor" class="actions">
                                <input type="number" min="0" value="{{ $single->retur_qty ?? null }}" name="qty[{{ $key }}][qty]" />
                            </td>
                        </tr>
