        ...
        <div class="card-table">
            <div class="form-table-container">
                <form id="filter-form" class="form-table-filter" method="GET" action="{{ route(module('getData')) }}">
                    <div class="row">
                        <x-input :col="2" name="retur_tanggal" type="date" placeholder="Search by Tanggal" :value="request('retur_tanggal')"/>
                        <x-input :col="4" name="retur_code" type="text" placeholder="Search by Code" :value="request('retur_code')"/>
                        ...
                        <table class="data-table">
                            <thead>
                                <tr>
                                    ...
                                    <x-th column="retur_code" text="Code" />
                                    <x-th :sortable=true column="retur_rs" text="Customer" />
                                    <x-th :sortable=true column="retur_tanggal" text="Tanggal" />
                                    <x-th column="retur_qty" text="Qty" />
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="hide-lg">
                                ...
                                @forelse($data as $list)
                                    <tr>
                                        ...
                                        <x-td field="retur_code" :model="$list" />
                                        <x-td field="customer_nama" :model="$list" />
                                        <td data-label="Tanggal">{{ formatDate($list->retur_tanggal) }}</td>
                                        <x-td field="retur_qty" :model="$list" />
                                    </tr>
                                @empty
                                    <tr>
