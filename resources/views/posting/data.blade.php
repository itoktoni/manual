<x-layout>
    ...
    <x-card title="{{ ucfirst('posting') }}">
        <div class="card-table">
            <div class="form-table-container">
                <form id="filter-form" class="form-table-filter" method="GET" action="{{ route(module('getData')) }}">
                    <div class="row">
                        <x-input name="customer_nama" type="text" placeholder="Search by Rs" :value="request('customer')" col="6"/>
                        <x-input name="jenis_nama" type="text" placeholder="Search by Jenis" :value="request('jenis')" col="6"/>
                    </div>
                    <div class="row">
                        <x-select name="perpage" :options="['10' => '10', '20' => '20', '50' => '50', '100' => '100']" :value="request('perpage', 10)" col="2" id="perpage-select"/>
                        <x-select name="filter" :options="['' => 'All Filter', 'customer' => 'Rs', 'jenis' => 'Jenis']" :value="request('filter')" col="4"/>
                        ...
                        <table class="data-table">
                            <thead>
                                <tr>
                                    ...
                                    <x-th column="customer" text="Customer" />
                                    <x-th column="jenis" text="Jenis" />
                                    <x-th column="tanggal" text="Tanggal" />
                                    <x-th column="type" text="Type" />
                                    <x-th column="kotor" text="Kotor" />
                                    <x-th column="qc" text="Qc" />
                                    <x-th column="bersih" text="Bersih" />
                                    <x-th column="pending" text="Pending" />
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="hide-lg">
                                ...
                                @forelse($data as $list)
                                    <tr>
                                        ...
                                        <x-td field="type" :model="$list" />
                                        <x-td field="kotor" :model="$list" />
                                        <x-td field="qc" :model="$list" />
                                        <x-td field="bersih" :model="$list" />
                                        <x-td field="pending" :model="$list" />
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="12" class="text-center">No posting found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
